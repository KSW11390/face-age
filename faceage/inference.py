...
def parse_args():  # 파싱 코드
    ...
    parser.add_argument(
        "--model_type", type=str, default="resnet", choices=["vgg", "resnet"]
    )
...
def load_model_and_head(args):  # 모델과 헤드를 로드
    print(
        f"[*] 모델 생성 중: {args.model_type} (Width: {args.width}, Act: {args.activation})"
    )
    ...
    head = SoftHead(args.feat_dim, num_bins=91, use_race=args.use_race_onehot).to(
        args.device
    )
...
def main():  # 모델과 헤드 로드 -> 정방향 패스 -> 추론 -> 결과출력
    ...
    with torch.no_grad():
        for img_path in image_files:
            ...
            info_text = f"Prediction : {expected_age:.1f} years\n"
