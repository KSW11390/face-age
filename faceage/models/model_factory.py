...
class ResNetStyle(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(
                in_channels, width, kernel_size=7, stride=2, padding=3, bias=False
            ),
        ...
        self.stage1 = nn.Sequential(
            BasicBlockExactHalf(
                chs[0], chs[0], stride=1, activation=act, dropout=dropout
            ),
            BasicBlockExactHalf(
                chs[0], chs[0], stride=1, activation=act, dropout=dropout
            ),
        ...
        self.stage2 = nn.Sequential(
            BasicBlockExactHalf(
                chs[0], chs[1], stride=2, activation=act, dropout=dropout
            ),  # 50->25
            BasicBlockExactHalf(
                chs[1], chs[1], stride=1, activation=act, dropout=dropout
            ),
        )
        self.stage3 = nn.Sequential(
            BasicBlockExactHalf(
                chs[1], chs[2], stride=2, activation=act, dropout=dropout
            ),  # 25->12
            BasicBlockExactHalf(
                chs[2], chs[2], stride=1, activation=act, dropout=dropout
            ),
        )
        self.stage4 = nn.Sequential(
            BasicBlockExactHalf(
                chs[2], chs[3], stride=2, activation=act, dropout=dropout
            ),  # 12->6
            BasicBlockExactHalf(
                chs[3], chs[3], stride=1, activation=act, dropout=dropout
            ),
        )
